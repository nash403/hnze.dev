---
title: "Curriculum Vitae"
description: "CV d'Honoré Nintunze, développeur web full-stack passionné"
---

::h-resume-hero{fullName="Honoré Nintunze" jobTitle="Full-Stack Web Developer" avatarUrl="/img/profile_picture.jpg" :phone="phone" :email="email" }
---
location:
  href: "https://maps.app.goo.gl/DpqeMMUGK5MJGYXd8"
  label: "Montreal H2T 1X2, QC, Canada"
  openInNewTab: true
  icon: clarity:map-marker-line

socialLinks:
  - type: linkedin
    username: nash403
    href: "https://www.linkedin.com/in/nash403"
    openInNewTab: true
    icon: bxl:linkedin-square

  - type: github
    username: nash403
    href: "https://github.com/nash403"
    openInNewTab: true
    icon: bxl:github

websiteLinks:
  - href: "https://hnze.dev"
    label: "hnze.dev"
    openInNewTab: true
    icon: clarity:link-line
---
#goal
Je suis un Ingénieur Web chevronné, spécialisé dans le développement full-stack. Passionné par l'open source, je m'engage activement à contribuer à la communauté avec curiosité et passion. Je m'attache à appliquer rigoureusement les meilleures pratiques de codage afin d'assurer la maintenabilité, la cohérence et la robustesse des projets auxquels je contribue. Fort d'une expérience diversifiée, je suis à l'aise avec l'écosystème JavaScript dans son ensemble, ainsi que divers outils de déploiement. Étant autodidacte et polyvalent, j'ai l'habitude d'endosser plusieurs rôles et je suis à la recherche de nouvelles opportunités pour innover.
::

<section>

## Expériences

::h-resume-work-experience
---
workPosition: "Projets personnels"
startDate: "2023-05-18"
endDate: "2024-05-22"
---
#summary
Pause sabbatique pour voyager, gérer des contraintes familiales et travailler sur des projets personnels tels que mon déménagement à Montréal.
::

::h-resume-work-experience
---
workPosition: "Développeur Nuxt.js"
companyName: "Zadig & Voltaire"
companyLogoUrl: "/img/companies/zv.svg"
companyLogoAlt: "Logo de Zadig & Voltaire"
companyUrl: "https://zadig-et-voltaire.com"
workLocation: "remote"
contractType: "freelance"
startDate: "2023-03-27"
endDate: "2023-05-17"
---
#summary
- Initialisation du socle technique en vue de la refonte du site E-commerce de Zadig&Voltaire. Migration Vue Storefront 2 vers Nuxt 3.
- Développement des composants du design system
- Création de modules Nuxt avancés pour la gestion du SEO, des robots.txt, la sitemaps, et d'autres encore.
::

::h-resume-work-experience
---
workPosition: "Ingénieur Web Full-Stack"
companyName: "Auchan Retail France"
companyLogoUrl: "/img/companies/auchan.svg"
companyLogoAlt: "Logo de Auchan Retail France"
companyUrl: "https://www.auchan-retail.com/fr"
workLocation: "Villeneuve d'Ascq (59), France"
contractType: "freelance"
startDate: "2023-01-18"
endDate: "2023-03-10"
---
#summary
- Maintenance et ajout de fonctionnalités à l'application de logistique Odyssée
- Création de micro-services Nest.js pour traiter en masse les événements de création ou modification de filière logistique via Pub/Sub de GCP.
- Garant de la qualité et du design (Tailwind, Jest, etc...)
- Architecture du nouveau front en Nuxt 3 en vue de la refonte de applicatif.
::

::h-resume-work-experience
---
workPosition: "Développeur Vue.js/Node.js"
companyName: "Décathlon"
companyLogoUrl: "/img/companies/decathlon.svg"
companyLogoAlt: "Logo de Décathlon"
companyUrl: "https://www.decathlon.fr/"
workLocation: "Marcq-en-Barœul (59), France"
contractType: "freelance"
startDate: "2022-09-26"
endDate: "2022-12-30"
---
#summary
Mise en place d'une plateforme de fitness pour la marque CORENGTH pour accompagner les utilisateurs à trouver des exercices d'entraînement adaptés à leur matériel. Développement d'une application mobile en Progressive Web App utilisant les technologies de player vidéo de Brightcove, le framework Nuxt, le CMS Strapi et des micro-services Node.js.
::

::h-resume-work-experience
---
workPosition: "Professeur de Programmation"
companyName: "EFFICOM"
companyLogoUrl: "/img/companies/efficom.png"
companyLogoAlt: "Logo de EFFICOM"
companyUrl: "https://www.efficom.fr/campus/lille/"
workLocation: "Lille (59), France"
contractType: "freelance"
startDate: "2021-10-26"
endDate: "2022-01-18"
---
#summary
Enseignement de l'initiation à la programmation à des étudiants de seconde année.
::

::h-resume-work-experience
---
workPosition: "Développeur Lead"
companyName: "Norauto International"
companyLogoUrl: "/img/companies/norauto.svg"
companyLogoAlt: "Logo de Norauto International"
companyUrl: "https://www.norauto.fr/"
workLocation: "Villeneuve d'Ascq (59), France"
contractType: "freelance"
startDate: "2020-11-02"
endDate: "2022-01-28"
---
#summary
Leader technique dans la conception et développement d'un nouveau site web e-commerce multi-langue / multi-pays pour Norauto International.

- Prototypage, comparaison et choix techniques: CMS, librairies, déploiements, versionning, etc…
- Participation au développement des composants et pages Nuxt 2.
- Intégration du CMS Contentful.
- Garent de la qualité et du design: écriture des tests, monitoring, bonnes pratiques, design system, etc…
- Entretien techniques de recrutement, accueil et intégration des nouveaux développeurs. Jusqu'à être référent d'une équipe de 9 développeurs.
::

::h-resume-work-experience
---
workPosition: "Développeur Web"
companyName: "Auchan Retail France"
companyLogoUrl: "/img/companies/auchan.svg"
companyLogoAlt: "Logo de Auchan Retail France"
companyUrl: "https://www.auchan-retail.com/fr"
workLocation: "Villeneuve d'Ascq (59), France"
contractType: "freelance"
startDate: "2020-05-04"
endDate: "2020-10-30"
---
#summary
- Participation au développement de multiples applications web à destination des employés et/ou des managers de Auchan Retail France à la centrale opérationnelle
- Harmonisation des pratiques chez Auchan: création de templates, librairies, guides.
::

::h-resume-work-experience
---
workPosition: "Développeur d'Applications Web"
companyName: "Adeo"
companyLogoUrl: "/img/companies/adeo.svg"
companyLogoAlt: "Logo de Adeo"
companyUrl: "https://www.adeo.com"
workLocation: "Villeneuve d'Ascq (59), France"
contractType: "freelance"
startDate: "2019-05-21"
endDate: "2020-04-03"
---
#summary
Développement de Hommy, un carnet de santé digital de la maison.
- Migration d'un applicatif devenu trop complexe et plein de bugs: Création du nouveau socle technique de l'application mobile PWA en Vue.js et Ionic Framework et pipelines de déploiement.
- Développement des pages et composants
Développement de MDHData, un catalogue produit interne pour les unités business de ADEO. Utilisé mensuellement par un millier d'employés.
- Développement des composants et pages de l'applicatif en Nuxt 2 avec du SSR, Tailwind, Vuetify, et Storybook.
- Dévelopments de endpoints backend avec la philosophie clean architecture en Nest.js, du CQRS, DDD, Event Sourcing, Elastic Search. Le tout en Typescript de bout en bout.
::

::h-resume-work-experience
---
workPosition: "Consultant Ingénieur Logiciel"
companyName: "Auchan Retail France"
companyLogoUrl: "/img/companies/auchan.svg"
companyLogoAlt: "Logo de Auchan Retail France"
companyUrl: "https://www.auchan-retail.com/fr"
workLocation: "Villeneuve d'Ascq (59), France"
contractType: "CDI"
startDate: "2017-09-04"
endDate: "2019-05-17"
---
#summary
- Participation au développement de multiples applications web à destination des employés et/ou des managers de Auchan Retail France à la centrale opérationnelle
- Harmonisation des pratiques chez Auchan: création de templates, librairies, guides.
- Refonte totale de l'application Pilot de visualisation de chiffre d'affaire et volume de vente utilisée par près de 11.000 employés.
::

::h-resume-work-experience
---
workPosition: "Alternant Développeur Full-Stack"
companyName: "Urbilog"
companyLogoUrl: "/img/companies/urbilog.png"
companyLogoAlt: "Logo de Urbilog"
companyUrl: "https://urbilog.com/"
workLocation: "Villeneuve d'Ascq (59), France"
contractType: "Alternance"
startDate: "2015-09-01"
endDate: "2017-06-30"
---
#summary
En tant qu'alternant à Urbilog, conception et développement de prototypes innovants autour des technologies web en intégrant l'accessibilité numérique.
::

::h-resume-work-experience
---
workPosition: "Stagiaire en développement Web"
companyName: "Urbilog"
companyLogoUrl: "/img/companies/urbilog.png"
companyLogoAlt: "Logo de Urbilog"
companyUrl: "https://urbilog.com/"
workLocation: "Villeneuve d'Ascq (59), France"
contractType: "Stage"
startDate: "2015-03-30"
endDate: "2015-07-31"
---
#summary
Développement d'une plateforme web de communication multi-canal à destination des personnes malvoyantes ou malentendantes.
::
</section>

<section>

## Compétences clés

### Langues parlées

::h-prose-element{.flex .flex-wrap .gap-2}
  [Français (Natif)]{.badge .badge-info}
  [Anglais (Professionnel)]{.badge .badge-info}
  [Kirundi (Élémentaire)]{.badge .badge-info}
::

### Programmation

::h-prose-element{.flex .flex-wrap .gap-2}
  [Typescript / Javascript / JSX]{.badge .badge-info}
  [HTML]{.badge .badge-info}
  [CSS / TailwindCSS]{.badge .badge-info}
  [JAVA]{.badge .badge-info}
  [Python]{.badge .badge-info}
  [GraphQL]{.badge .badge-info}
  [REST API]{.badge .badge-info}
  [SQL / NoSQL]{.badge .badge-info}
::

### Frameworks & Web

::h-prose-element{.flex .flex-wrap .gap-2}
  [Vue.js / Nuxt.js]{.badge .badge-info}
  [Node.js / Bun]{.badge .badge-info}
  [Angular]{.badge .badge-info}
  [Contentful / Prismic / Strapi]{.badge .badge-info}
  [Nest.js]{.badge .badge-info}
  [Express.js]{.badge .badge-info}
  [Ionic]{.badge .badge-info}
  [SEO]{.badge .badge-info}
  [Core Web Vitals]{.badge .badge-info}
::

### Testing

::h-prose-element{.flex .flex-wrap .gap-2}
  [Jest]{.badge .badge-info}
  [Vitest]{.badge .badge-info}
  [ESlint]{.badge .badge-info}
  [Cypress]{.badge .badge-info}
  [Lighthouse]{.badge .badge-info}
  [Chai / Mocha / Sinon]{.badge .badge-info}
::

### Backend & Devops

::h-prose-element{.flex .flex-wrap .gap-2}
  [Docker]{.badge .badge-info}
  [CI/CD]{.badge .badge-info}
  [Kubernetes]{.badge .badge-info}
  [AWS]{.badge .badge-info}
  [GCP]{.badge .badge-info}
  [Vercel]{.badge .badge-info}
  [Websocket]{.badge .badge-info}
  [WebRTC]{.badge .badge-info}
  [Redis]{.badge .badge-info}
  [Postgres]{.badge .badge-info}
  [MongoDB]{.badge .badge-info}
  [SQLite]{.badge .badge-info}
::

### Méthodologie

::h-prose-element{.flex .flex-wrap .gap-2}
  [Méthodes Agiles (Scrum / Kanban)]{.badge .badge-info}
  [Tests unitaires]{.badge .badge-info}
  [Pair programming]{.badge .badge-info}
  [Code reviews]{.badge .badge-info}
  [Daily stand-ups]{.badge .badge-info}
  [Test Driven Development]{.badge .badge-info}
  [Domain Driven Development]{.badge .badge-info}
::
</section>

<section>

## Scolarité

::h-resume-certification
---
certificationName: "Master E-Services"
institutionName: "Université Lille 1 Sciences et Technologies"
institutionUrl: "https://www.univ-lille.fr/formations/fr-00002035"
dates: "2015 / 2017"
---
#summary

Formation spécialisée en développement Web et mobile. Acquis: maquettage, tests unitaire et d'interface, Android, Angular, Java EE, etc...
::

::h-resume-certification
---
certificationName: "Licence Informatique"
institutionName: "Université Lille 1 Sciences et Technologies"
institutionUrl: "https://www.univ-lille.fr/formations/fr-00002032"
dates: "2012 / 2015"
---
#summary

Programmation, Codage, Algorithmique, développement de sites internet
::

::h-resume-certification
---
certificationName: "Baccalauréat Scientifique"
institutionName: "École Internationale de Bujumbura"
dates: "2012"
---
#summary

Diplôme d'État du Burundi, avec mention Grand Fruit (= Très Bien).
::
</section>


<section>

## Intérêts

<div class="print:prose-xs prose prose-base md:prose-sm prose-li:list-[square] prose-li:marker:text-accent prose-li:[&_li]:list-['-']">

  - Voyages (PVT à Montreal, road trip aux USA, Europe, etc.)
  - Veille techologique (Newsletters, personnalités tech sur les réseaux sociaux)
  - Contributions Open Source
  - Meetups
  - Chaînes YouTube (Micode, Science Étonnante, Stupid Economics, etc.)
  - Séries TV et documentaires.

</div>
</section>
